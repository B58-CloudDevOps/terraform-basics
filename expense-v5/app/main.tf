...
resource "null_resource" "main" {
  ...
  provisioner "local-exec" {
    command = "sleep 10; cd /home/ec2-user/ansible ; ansible-playbook -i   -e ansible_user=ec2-user -e ansible_password=DevOps321 -e COMPONENT=${var.name} -e ENV=dev  expense.yml"
  }
}