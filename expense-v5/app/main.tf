...
resource "null_resource" "app" {
  ...
  provisioner "local-exec" {
    command = "sleep 120; cd /home/ec2-user/ansible ; ansible-playbook -i inv-dev  -e ansible_user=ec2-user -e ansible_password=DevOps321 -e COMPONENT=${var.name} -e ENV=dev -e PWD=${var.pwd} expense.yml"
  }
}
